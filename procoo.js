...
function Curseur(x, y, dir) {
	this.x = x || 0;
	this.y = y || 0;
	this.dir = dir || 0;
	this.placer = function (x, y) {
		this.x = x;
		this.y = y;
	};
	...
	this.tourner = function (dir) {
		this.dir = dir;
	};
	this.detourner = function (dir) {
		dir = dir || (hasard(2) * 2) - 1; // Donne +1 ou -1
		this.dir += (dir + 4) % 4;
	};
	this.retourner = function () {
		this.dir = (this.dir + 2) % 4;
	};
	this.avancer = function (pas) {
		pas = pas || 1;
		this.x += ([0, 1, 0, -1][this.dir]) * pas;
		this.y += ([-1, 0, 1, 0][this.dir]) * pas;
	};
}
...
function labyrinthe(largeur, hauteur) {
	...
	var c = new Curseur(Math.floor(largeur / 2), Math.floor(hauteur / 2), hasard(4));
	////var colonne = Math.floor(largeur/2);	// On démarre le curseur à un endroit aléatoire
	////var rangee = Math.floor(hauteur/2);		// On démarre le curseur à un endroit aléatoire
	////var dir = hasard(4);				// On détermine une direction au hasard. 0=nord; 1=est; 2=sud; 3=ouest
...
function labyrintheBK(largeur, hauteur) {
	var rangee; // Entre autres la position du curseur
	var colonne; // Entre autres la position du curseur
	var dir; // La direction pointée par le curseur
